...
export async function handleSubmission(id: number, userAgent: string, config: Config): Promise<HandlerResult> {
  ...
  if (requester === 'human') {
    return {
      ...
      meta: { requester, cached: null, submissionResult: 'redirect' },
    };
  }
    ...
    switch (result.type) {
      ...
      case 'accountDisabled':
        return {
          ...
          html: generateMessageEmbed(
            'Account Disabled',
            'Access to this account and its content has been disabled on FurAffinity',
          ),
          meta,
        };
      case 'blocked':
